        ...
        <table className="w-full text-xs">
          <thead>
            <tr className="bg-slate-50 border-b border-slate-100">
              <th className="text-left px-4 py-2 font-semibold text-slate-500">Dimension</th>
              <th className="text-center px-3 py-2 font-semibold text-slate-500 w-24">Result</th>
              <th className="text-left px-3 py-2 font-semibold text-slate-500">Note</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-slate-50">
            {evaluation.compatibility.map((c, i) => (
              <tr key={i} className="hover:bg-slate-50/50">
                <td className="px-4 py-2 font-medium text-slate-700">{c.dimension}</td>
